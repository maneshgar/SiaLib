...
def create_learning_rate_fn(num_epochs, warmup_epochs, base_learning_rate, steps_per_epoch):
  ...
  cosine_alpha =  0.05
  warmup_steps = min(2000, warmup_epochs*steps_per_epoch)
  warmup_fn = optax.linear_schedule(init_value=0.000001, end_value=base_learning_rate, transition_steps=warmup_steps)
